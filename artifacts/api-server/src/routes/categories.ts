...
// GET is public so the booking wizard can list categories without auth
router.get("/categories", async (_req, res): Promise<void> => {
  const categories = await db.select().from(categoriesTable).orderBy(categoriesTable.name);
  res.json(ListCategoriesResponse.parse(categories));
});

router.post("/categories", requireVendor, async (req, res): Promise<void> => {
...
router.patch("/categories/:id", requireVendor, async (req, res): Promise<void> => {
...
router.delete("/categories/:id", requireVendor, async (req, res): Promise<void> => {
